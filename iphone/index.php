	...
	<!-- Add an hidden form in the header. Will be shown pressing search button --> 
			...
			<div id="iGroup">
				...
				<form rev="async" action="iregister.php" id="registerForm" onSubmit="return WA.Submit('registerForm');">
					...
					<fieldset>
						<ul> 
							<li><input type="text" name="myname" placeholder="Your Name (Optional)" /></li>
							<li><input type="text" name="baddress" placeholder="Your Address (Optional)" /></li>
							...
							echo"<select name=\"userregion\">";
							$res = mssql_query("SELECT * FROM regions");
							for($i=0;$i<mssql_num_rows($res);$i++) {
								$row = mssql_fetch_assoc($res);
								echo "<option value=\"".$row["region_id"]."\">".$row["city"]."</option>";
							}
							echo'</select>';
